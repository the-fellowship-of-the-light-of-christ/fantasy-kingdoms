...
class CA_Getwood_Model extends Character_Action_Model
{
	// Costanti	
	const DELTA_GLUT = 5;
			...
			foreach ( $items as $key => $value )
			{
				foreach ( $par[0] -> structure_resource as $resource )
					if ( $resource -> resource == $key )				
						...
						{ $message = kohana::lang('ca_getwood.resourcenotenough'); return FALSE; }
			}		
		}
		else
		{
			$resource_status = $par[0] -> check_resource_status( 'wood_piece', 1 );		
			
			if ( $resource_status == false )
				{ $message = kohana::lang('ca_getwood.resourceisdepleted'); return FALSE; }
		}
	...
	protected function append_action( $par, &$message )
	{
		...
		$message = kohana::lang('ca_getwood.getwood-ok');
	...
	*/
	...
	public function complete_action( $data )
	{
		...
		$char->modify_energy ( - self::DELTA_ENERGY * $data->param3, false, 'getwood');
		...
		foreach ( $items as $item )
		{
			...
			else
			{
				...
				Character_Event_Model::addrecord( $char -> id, 'normal', '__events.cutwoodnotok', 'normal' );
			}
		}				
	
		// evento per quest
		$par[0] = $quantity;
		GameEvent_Model::process_event( $char, 'getwood', $par );
	...
	public function get_action_message( $type = 'long') 
	{
		...
		{
			if ( $type == 'long' )		
			$message = '__regionview.getwood_longmessage';
			else
			$message = '__regionview.getwood_shortmessage';
		}
		return $message;
